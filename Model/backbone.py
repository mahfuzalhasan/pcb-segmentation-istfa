...
class ResNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.maxpool(F.relu(self.bn1(self.conv1(x))))   # [1, 64, H/4, W/4] ->[1,64,128,128]
        print('input: ',x.size())                           
        x1 = self.layer1(x)  # [1, 64/256, H/4, W/4]   -> [1,64,128,128]
        print('layer_1: ',x1.size())
        x2 = self.layer2(x1)  # [1, 128/512, H/8, W/8] -> [1,128,64,64]
        print('layer_1: ',x2.size())
        x3 = self.layer3(x2)  # [1, 256/1024, H/16, W/16] -> [1,256,32,32]
        print('layer_1: ',x3.size())
        x4 = self.layer4(x3)  # [1, 512/2048, H/32, W/32]   -> [1,512,16,16]
        print('layer_1: ',x4.size())
        return x1, x2, x3, x4
